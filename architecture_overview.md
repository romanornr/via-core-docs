...
graph TB
    ...
    Users[Users/Applications] --> ExternalNode
    Users --> Mempool
    Mempool --> Sequencer
    Sequencer --> ExternalNode
...
The External Node is a read-replica of the main validator node. It continuously synchronizes state and metadata from the main node and serves JSON-RPC requests locally, offloading read traffic from the validator and enabling horizontal scaling of RPC access.

Key capabilities:
- Read-only RPC serving for state queries and historical lookups
- Optional local components for performance and observability (Merkle tree, consistency checker, BTC watcher, metrics exporters)
- Optional proxying or forwarding of transaction submissions to the main node

See [external_node.md](external_node.md) for implementation details and deployment guidance.
...
### 4.4 External Node Sync and Read Path

External Nodes periodically synchronize state and metadata from the main (validator) node and expose a local, read-optimized JSON-RPC interface. They do not participate in sequencing, proving, or finalization. If transaction submission endpoints are exposed, writes are forwarded to the main node; all authoritative state transitions occur on the validator.

Refer to [external_node.md](external_node.md) for a deeper dive.
